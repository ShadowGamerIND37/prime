...
async function main() {
  ...
  log('⚡ Initializing Prime Minecraft Automated Setup...');

  // 1. Ensure required runtime directories exist
  const dirs = [
    path.join(rootDir, 'data'),
    path.join(rootDir, 'prime/servers'),
    path.join(rootDir, 'prime/backups'),
  ...
  if (!fs.existsSync(envPath)) {
    ...
    const envContent = `# Prime Minecraft Configuration Environment
NODE_ENV=production
PORT_WEB=3001
PORT_API=3003
PORT_SFTP=3004
...
`;
  ...
  if (options.adminUser && options.adminPass) {
    ...
    const email = options.adminEmail ? options.adminEmail.trim() : `${username}@prime-minecraft.local`;
  ...
  if (options.json) {
    ...
    log('✅ Prime Minecraft automated setup completed successfully!');
  }
...
main().catch((err) => {
  console.error('❌ Prime Minecraft setup error:', err);
  process.exit(1);
});
