...
class ServerConsole {
  ...
  connectWebSocket() {
    ...
    this.ws.onopen = () => {
      if (this.term) {
        this.term.writeln('\x1b[32m[Prime Minecraft]\x1b[0m Connected to server live stream.');
      }
    };
    ...
    this.ws.onclose = () => {
      if (this.term) {
        this.term.writeln('\r\n\x1b[33m[Prime Minecraft]\x1b[0m Disconnected from server stream.');
      }
    };
  }
    ...
    container.innerHTML = `
      <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="glass-panel p-6 rounded-3xl border border-white/10 space-y-4">
          ...
          <p class="text-[11px] text-slate-400">Password is your regular Prime Minecraft account login password.</p>
        </div>
