...
router.post('/', authenticate, async (req, res) => {
  try {
    ...
    if (server_type === 'minecraft') {
      ...
      fs.writeFileSync(path.join(serverDir, 'index.js'), `// Prime Minecraft Node.js Application\nconst http = require('http');\nconst port = process.env.PORT || 3000;\n\nconst server = http.createServer((req, res) => {\n  res.writeHead(200, { 'Content-Type': 'text/plain' });\n  res.end('Hello from Prime Minecraft Node.js Server! Port: ' + port);\n});\n\nserver.listen(port, () => {\n  console.log('App running on port ' + port);\n});\n`, 'utf8');
    } else if (server_type === 'python') {
      fs.writeFileSync(path.join(serverDir, 'requirements.txt'), '# Add your Python dependencies here\nflask\n', 'utf8');
      fs.writeFileSync(path.join(serverDir, 'app.py'), `# Prime Minecraft Python Application\nimport os\nfrom http.server import HTTPServer, BaseHTTPRequestHandler\n\nport = int(os.environ.get('PORT', 5000))\n\nclass Handler(BaseHTTPRequestHandler):\n    def do_GET(self):\n        self.send_response(200)\n        self.send_header('Content-type', 'text/plain')\n        self.end_headers()\n        self.wfile.write(b'Hello from Prime Minecraft Python App!')\n\nprint(f"Starting Python server on port {port}...")\nhttpd = HTTPServer(('0.0.0.0', port), Handler)\nhttpd.serve_forever()\n`, 'utf8');
    }
