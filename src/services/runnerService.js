  ...
  syncMinecraftProperties(serverDir, targetPort) {
    ...
    try {
      ...
      if (!fs.existsSync(eulaPath)) {
        fs.writeFileSync(eulaPath, '# EULA accepted by Prime Minecraft\neula=true\n', 'utf8');
      } else {
      ...
      console.warn('[Prime Minecraft] Warning updating eula.txt:', err.message);
    }
    ...
    try {
      ...
      if (!fs.existsSync(propsPath)) {
        const defaultProps = [
'# Minecraft server properties generated by Prime Minecraft',
'motd=Powered by Prime Minecraft',
          `server-port=${port}`,
          `query.port=${port}`,
          'server-ip=',
          'online-mode=true',
      ...
      console.warn('[Prime Minecraft] Warning updating server.properties:', err.message);
    }
  }
  ...
  async startServer(serverId) {
    ...
    this.appendLog(sId, `\r\n\x1b[36m[Prime Minecraft]\x1b[0m Starting server ${server.name}...\r\n`);
    ...
    if (server.server_type === 'minecraft') {
      ...
      if (!fs.existsSync(jarPath)) {
        this.appendLog(sId, `\x1b[33m[Prime Minecraft]\x1b[0m server.jar missing! Automatically downloading ${server.jar_type || 'paper'} (${server.jar_version || '1.21.4'})...\r\n`);
        try {
          ...
          this.appendLog(sId, `\x1b[32m[Prime Minecraft]\x1b[0m Successfully installed server.jar!\r\n`);
        } catch (jarErr) {
          this.appendLog(sId, `\x1b[31m[Prime Minecraft Error]\x1b[0m Could not download server.jar: ${jarErr.message}\r\n`);
        }
      }
    }
    ...
    if (isMinecraft) {
      ...
      this.appendLog(sId, `\x1b[36m[Prime Minecraft]\x1b[0m Synchronized Minecraft port: server-port=${targetPort}, query.port=${targetPort}\r\n`);
    }
    ...
    if (dockerService.isAvailable) {
      try {
        this.appendLog(sId, `\x1b[32m[Prime Minecraft]\x1b[0m Containerizing with ${server.docker_image}...\r\n`);
        ...
        this.appendLog(sId, `\x1b[33m[Prime Minecraft Warning]\x1b[0m Docker spawn issue (${dockErr.message}). Switching to native runner.\r\n`);
      }
    }

    // Fallback native process runner
    try {
      this.appendLog(sId, `\x1b[32m[Prime Minecraft]\x1b[0m Executing command: ${startupCmd}\r\n`);
      ...
      child.on('close', (code) => {
        this.appendLog(sId, `\r\n\x1b[31m[Prime Minecraft]\x1b[0m Server process stopped with exit code ${code}.\r\n`);
        record.process = null;
        record.status = 'offline';
        query.run('UPDATE servers SET status = ? WHERE id = ?', ['offline', sId]);
        this.broadcast(sId, { type: 'status', status: 'offline' });
      ...
      child.on('error', (err) => {
        this.appendLog(sId, `\r\n\x1b[31m[Prime Minecraft Error]\x1b[0m ${err.message}\r\n`);
      });
  ...
  async stopServer(serverId) {
    ...
    this.appendLog(sId, `\r\n\x1b[33m[Prime Minecraft]\x1b[0m Stopping server...\r\n`);
  ...
  async killServer(serverId) {
    ...
    await query.run('UPDATE servers SET status = ? WHERE id = ?', ['offline', sId]);
    this.broadcast(sId, { type: 'status', status: 'offline' });
    this.appendLog(sId, `\r\n\x1b[31m[Prime Minecraft]\x1b[0m Server was forcefully terminated.\r\n`);
    return { success: true };
  }
    ...
    try {
      ...
      for (const c of containers) {
        for (const name of (c.Names || [])) {
          const match = name.match(/prime-minecraft-server-(\d+)-/);
          if (match) {
  ...
  async sendCommand(serverId, command) {
    ...
    if ((!record || record.status !== 'running') && dockerService.isAvailable && dockerService.docker) {
      try {
        const containers = await dockerService.docker.listContainers();
        const match = containers.find(c => (c.Names || []).some(n => n.includes(`prime-minecraft-server-${sId}-`)));
        if (match) {
